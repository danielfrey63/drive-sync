...
- **Retention** (`forget` after every run): every state for 7 days, then one per day for 30 days, then one per month for 12 months. `prune` frees the chunks no snapshot references any more and `check --read-data-subset=2%` reads back a random 2 % of the data — both on Sundays only, because they rewrite the index over SFTP. With **several machines in one repository**, set `MaintenanceHost` to the `COMPUTERNAME` of the one that owns prune: the lock is exclusive, and the loser of that race would report a failure for something that is not one. `forget` stays on every machine, since `--group-by host,paths` gives each host its own chains.
- **Suspend tolerance:** the laptop is put to sleep on purpose whenever it travels, and a suspend mid-run kills the ssh session. A failed source is retried on a widening ladder (1, 3, 7, 15, 30 min), and each step waits *for the box to answer* rather than for the clock, so a short nap costs one poll and a long one uses the budget. After that the run reports a **postponement**, not a failure: the next slot continues from the repository index. Only a missing network counts as that — a reachable box whose ssh refuses (key gone from the agent, host key changed, SSH-Support switched off) is a real failure and says so. Four postponements in a row, a full day without a backup, escalate to a failure as well.
...
- **`Backup postponed`** — the box did not answer on TCP, so the machine was asleep or off the network. Nothing to do; the next slot continues. The log line carries the streak, and four in a row turn into a failure. In the log this looks like `ssh command exited: exit status 255` on the lock operations, often together with `failed to delete VSS snapshot: VSS error: async operation pending` — that orphan is released by Windows, `vssadmin list shadows` only shows the persistent `ClientAccessibleWriters` restore points.
