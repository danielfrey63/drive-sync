...
1. **Nothing stale on the new machine before the first resync.** `--resync` merges and never deletes: any file that exists only locally is uploaded again, including everything that was deliberately deleted elsewhere since that copy was made (676 revived files on the first attempt). `sync-drive.ps1 -Resync` therefore runs `resync-guard.ps1` first: the Drive trash is the deletion log — every local file that matches a trashed cloud file by path and modtime (and is not live in the cloud again) goes to the recycle bin before the resync starts (`-DryRun` reports only, `-SkipResyncGuard` bypasses). Reach is the trash retention of 30 days; for older stale copies start from an empty local root or run `rclone check "<local>" gdrive: --one-way --missing-on-dst stale.txt --filter-from filters.txt` and delete what it lists.
